# HANDOFF_04 — Family 4 Recipe Authoring

**Project:** Pamplin AI Agent Recipes (`teaching-agents`)
**Spec reference:** `SPEC.md` v0.1.2
**Cut:** Content authoring — real recipe Instructions text and customization notes for the three Family 4 course-architecture recipes, including the catalog's third and final Heavy-tier recipe.

---

## Goal of this iteration

Replace the placeholder Instructions, framing paragraphs, and customization notes for the three Family 4 recipes. Flip `content_status` to `"final"` for these three. Rebuild and verify.

After this handoff, 13 of 23 recipes ship with real content. The remaining 10 recipes (Families 3, 5, 6, 7) stay on placeholder content with their DRAFT banners visible.

Mechanically identical to HANDOFF_03: no schema, template, or build pipeline changes. The infrastructure from HANDOFF_02 already supports everything this handoff needs.

---

## Why Family 4 next (and not Family 3)

The original SPEC §12 sequencing put Family 3 next. We've adjusted to Family 4 first because:

- Family 4's recipe 4.1 (Course Format Converter) is the only Heavy-tier recipe in the catalog that hasn't been authored yet. Heavy recipes are the aspirational anchors — they show faculty what's possible at the upper bound. Authoring 4.1 now means faculty browsing the catalog see that bound represented before more middle-tier content piles up.
- Family 3 (Discussion and case-method) is conceptually adjacent to recipes already shipped (1.2 Live Case-Discussion Facilitator, 2.2 Concept Tutor). It can wait without leaving the catalog feeling unrepresented.

Family 3 follows in HANDOFF_05.

---

## Authoring approach (for context, not action)

Same approach as HANDOFF_02 and HANDOFF_03, established in SPEC v0.1.2 §7:

- **Complete out of the box** — Instructions are a fully working system prompt with realistic example values pre-filled.
- **Guillemet markers `«...»`** for customization slots, preserved verbatim in JSON.
...
- **Customization notes structure**: brief intro line → "Quick swaps" bullets → "Behavioral customizations" bullets. Sub-bullets at two-space indent.

The embedded course examples for Family 4 are: **FIN 4014 Investments** (4.1), **MGT 4304 Organizational Behavior** (4.2), **MKTG 3104 Marketing Management** (4.3). These were chosen to add a second Finance, third Management, and second Marketing example — bringing the cumulative distribution closer to matching the workshop's submission weights (Management 26, Finance 21, BIT 17 were the top three).

A note on recipe 4.1: it's a Heavy recipe, intended for sustained use with a full course's worth of uploaded materials. Its Instructions text is closer to the 7,500-character limit than other recipes (~6,800 chars in the draft below) because the recipe needs to specify behavior across multiple conversion modes (semester-to-summer, in-person-to-async, etc.) and across multiple kinds of source material (syllabus, schedule, slides, assignments). Within budget but tight.
...
```
recipes/014-course-format-converter.json
recipes/015-syllabus-modernizer.json
recipes/016-module-architect.json
```
...
For each of the three Family 4 recipes:
...
- The three Family 4 recipe pages show: real Instructions, no DRAFT banner, customization notes section.
- The 10 already-final recipe pages (Families 1, 2) still render correctly (no regression).
- The remaining 10 placeholder recipe pages still show placeholder Instructions and DRAFT banners.
...
```
HANDOFF_04: Family 4 recipe content authored

- Replaces placeholder Instructions, framing paragraphs, and adds
  customization notes for the three course-architecture recipes
  (4.1 through 4.3)
- Includes the catalog's third Heavy-tier recipe (4.1 Course
  Format Converter)
- Flips content_status to "final" for these three recipes
- 13 of 23 recipes now have real content; 10 remain on placeholder
```
...
**File:** `recipes/014-course-format-converter.json`
...
**File:** `recipes/015-syllabus-modernizer.json`
...
```
You are a syllabus modernization assistant for «MGT 4304: Organizational Behavior», an undergraduate course at Virginia Tech's Pamplin College of Business taught by «Professor Diaz».
...
```
The Instructions are filled in with example values for **MGT 4304: Organizational Behavior**. To customize for your course, search the Instructions text for `«` and you'll find every customization point. The behavior is course-agnostic; what changes is just the embedded example identity.
...
**File:** `recipes/016-module-architect.json`
...
```
The Instructions are filled in with example values for **MKTG 3104: Marketing Management**. To customize for your course, search the Instructions text for `«` and you'll find every customization point. The recipe is largely course-agnostic — the structural design pattern works across disciplines.
...
- **Preserve the guillemet markers verbatim.** Part of the recipe content.
- **Preserve line breaks within the Instructions text.** Headings and lists are intentional.
...
- **No changes outside the three Family 4 recipes.** Don't touch Families 1, 2, or any still-placeholder recipe.
- **No schema, template, or build pipeline changes.** Infrastructure unchanged from HANDOFF_02.
...
**Content:**
- [ ] All three Family 4 recipe JSON files have real `framing_paragraph`, `fields.instructions`, `customization_notes`, and `content_status: "final"`, all matching this handoff verbatim.
...
**Visual verification:**
- [ ] All three Family 4 recipe pages show: real Instructions, no DRAFT banner, customization notes section.
- [ ] All 10 Family 1+2 recipe pages still render correctly (no regression).
- [ ] Two sampled placeholder recipes (e.g., from Families 3, 5) still show: placeholder Instructions, DRAFT banner.
- [ ] Catalog home page renders identically to before.

**Hygiene:**
- [ ] Single commit with the message specified in D3.
- [ ] CC's final report includes: confirmation of all done criteria, sample HTML excerpt of one of the new Family 4 recipe pages, any decisions made.

---

## Notes for CC

- **Same operational pattern as HANDOFF_02 and HANDOFF_03.** Copy verbatim. No content editing. The `tools/_apply_handoff_03_content.py` pattern works directly here — write `tools/_apply_handoff_04_content.py` with the same structure.
- **Three recipes is fewer than HANDOFF_03's six** — should be the smallest content handoff so far.
- **Recipe 4.1's Instructions are close to the 7,500 limit** (~6,800 chars). All three recipes are within budget; CC should not need to truncate or edit. If any recipe exceeds 7,500 characters in the JSON, surface it in the final report.
- **Spot-check a Family 1 and a Family 2 page after rebuild** to confirm no regression — the rebuild touches all 23 pages.

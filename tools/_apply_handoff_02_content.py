#!/usr/bin/env python3
"""
One-shot script for HANDOFF_02: writes verbatim recipe content from
HANDOFF_02.md into the four Family 2 recipe JSON files.

Run from repo root:
    python tools/_apply_handoff_02_content.py

This script is intentional and conservative: it loads each existing recipe
JSON, replaces only `framing_paragraph` and `fields.instructions`, and adds
`customization_notes` and `content_status: "final"`. All other fields are
preserved untouched. It uses json.dumps(ensure_ascii=False) so guillemets
land as UTF-8 in the file.
"""
...
R_007_FRAMING = (
    "This recipe builds an agent that answers your students' logistics "
    "questions — deadlines, policies, where to find materials, what "
    "counts as late, how grades are calculated — based on documents you "
    "upload (your syllabus, schedule, assignment descriptions). The agent "
    "stays inside what your sources actually say and refers students back to "
    "you when the answer isn't there. The recipe is light enough that you "
    "can build the agent in 15 minutes from a single uploaded syllabus, and "
    "the agent grows in usefulness as you add more sources over the semester."
)

R_007_INSTRUCTIONS = """You are a course FAQ assistant for «FIN 3104: Introduction to Finance», an undergraduate course taught by «Professor Smith» at Virginia Tech's Pamplin College of Business.
...
R_007_CUSTOMIZATION = """The Instructions are filled in with example values for **FIN 3104: Introduction to Finance**. To customize for your course, search the Instructions text for `«` and you'll find every customization point. Quick swaps below; behavioral customizations follow.
...
R_008_FRAMING = (
    "This recipe builds an agent that helps students build conceptual "
    "understanding of course material — analogies, walkthroughs, \"what "
    "does this mean\" reframings — without giving away problem-set "
    "answers or doing graded work for them. The guardrail (refusing to solve "
    "graded problems while still helping the student understand the "
    "underlying concepts) is the load-bearing feature, and the Instructions "
    "are written to hold that guardrail under pressure."
)

R_008_INSTRUCTIONS = """You are a concept tutor for «MGT 3304: Strategic Management», an undergraduate course at Virginia Tech's Pamplin College of Business taught by «Professor Lee».
...
R_008_CUSTOMIZATION = """The Instructions are filled in with example values for **MGT 3304: Strategic Management**. To customize for your course, search the Instructions text for `«` and you'll find every customization point.
...
R_009_FRAMING = (
    "This recipe builds an agent that asks students conceptual questions "
    "about course material, listens to their answers, and adjusts its "
    "follow-up questions based on what they understood or missed. It's the "
    "inverse of a tutor — instead of explaining concepts to students, "
    "it quizzes them in a Socratic way and helps them discover where their "
    "understanding is solid and where it's shaky. The Instructions are "
    "unusually behavior-focused: most of the text is about *how* to run the "
    "dialogue, not what to say."
)

R_009_INSTRUCTIONS = """You are a concept-practice partner for «BIT 3414: Business Database Management», an undergraduate course at Virginia Tech's Pamplin College of Business taught by «Professor Patel».
...
R_009_CUSTOMIZATION = """The Instructions are filled in with example values for **BIT 3414: Business Database Management**. To customize for your course, search the Instructions text for `«` and you'll find every customization point.
...
R_010_FRAMING = (
    "This recipe builds an agent that students use throughout the semester "
    "for review, clarification, and asynchronous study support. It's the "
    "broadest of the student-facing agents — combining the "
    "logistics-answering of the FAQ recipe (2.1) with the concept-tutoring "
    "of the concept tutor (2.2), grounded on materials you upload (slides, "
    "readings, syllabus, past assignments). The Instructions are designed "
    "to make the agent useful immediately while staying within the "
    "boundaries of what your course materials actually cover."
)

R_010_INSTRUCTIONS = """You are the course assistant for «HTM 4474: Hospitality Revenue Management», an undergraduate course at Virginia Tech's Pamplin College of Business taught by «Professor Garcia».
...
R_010_CUSTOMIZATION = """The Instructions are filled in with example values for **HTM 4474: Hospitality Revenue Management**. To customize for your course, search the Instructions text for `«` and you'll find every customization point.
...
UPDATES = [
    ("007-course-faq-answerer.json",            R_007_FRAMING, R_007_INSTRUCTIONS, R_007_CUSTOMIZATION),
    ("008-concept-tutor-no-answers.json",       R_008_FRAMING, R_008_INSTRUCTIONS, R_008_CUSTOMIZATION),
    ("009-adaptive-concept-practice-partner.json", R_009_FRAMING, R_009_INSTRUCTIONS, R_009_CUSTOMIZATION),
    ("010-reusable-course-assistant.json",      R_010_FRAMING, R_010_INSTRUCTIONS, R_010_CUSTOMIZATION),
]

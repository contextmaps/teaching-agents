#!/usr/bin/env python3
"""
One-shot script for HANDOFF_07: writes verbatim recipe content from
HANDOFF_07.md into the four remaining placeholder recipe JSON files
(three in Family 6, one in Family 7).

Run from repo root:
    python tools/_apply_handoff_07_content.py

Mirrors tools/_apply_handoff_06_content.py: loads each existing recipe JSON,
...
R_020_FRAMING = (
    "This recipe builds an agent that takes a concept you're teaching "
    "and produces a set of mini-cases or examples tuned to a specific "
    "industry, student level, or current relevance — varying difficulty "
    "and varying angle, ready to drop into a lecture. It's a one-shot "
    "generator: you tell the agent the concept and the constraints, the "
    "agent produces 3-5 different examples for you to pick from. The "
    "example below is set up for an IT Project Management course, but "
    "the recipe works for any course where you'd benefit from quickly "
    "getting multiple takes on a teaching example."
)

R_020_INSTRUCTIONS = """You are an example generator for «BIT 4524: IT Project Management», an undergraduate course at Virginia Tech's Pamplin College of Business taught by «Professor Saito».
...
R_020_CUSTOMIZATION = """The Instructions are filled in with example values for **BIT 4524: IT Project Management**. To customize for your course, search the Instructions text for `«` and you'll find every customization point. The recipe is largely course-agnostic — the example-generation pattern works across disciplines.
...
R_021_FRAMING = (
    "This recipe builds an agent that takes a recent news event — you "
    "paste a link or summary — and translates it into a mini-case or "
    "in-class discussion vehicle for a specific course. The agent "
    "extracts the framing question, the key tensions, and the "
    "connection to course concepts, producing a teaching artifact you "
    "can use the day the news is fresh. It's the catalog's most "
    "time-sensitive recipe: the example case loses freshness in weeks. "
    "Use it to bring real-world relevance into class without re-reading "
    "the case literature every semester. The example below is set up "
    "for a Digital Marketing Strategy course, but the recipe works for "
    "any course where current events are pedagogically valuable."
)

R_021_INSTRUCTIONS = """You are a current-events case translator for «MKTG 4424: Digital Marketing Strategy», an undergraduate course at Virginia Tech's Pamplin College of Business taught by «Professor Reyes».
...
R_021_CUSTOMIZATION = """The Instructions are filled in with example values for **MKTG 4424: Digital Marketing Strategy**. To customize for your course, search the Instructions text for `«` and you'll find every customization point. The recipe is largely course-agnostic — the news-translation pattern works across disciplines.
...
R_022_FRAMING = (
    "This recipe builds an agent that takes a concept and explains it "
    "through the lens of multiple business disciplines — for instance, "
    "\"explain risk\" with framings from Finance, Marketing, Real "
    "Estate, and Management — so faculty can either pick the framing "
    "that fits their course or use the contrast itself as the teaching "
    "moment. It's the catalog's second Level 3 cross-disciplinary "
    "recipe (after 1.5 Hands-On Data Activity Builder); the "
    "multi-framing IS the recipe's value, not a side feature. The "
    "example below is anchored in a Real Estate Finance course, with "
    "the agent producing framings from Finance, Marketing, and "
    "Management for the concept of \"leverage.\" Customize the "
    "anchoring discipline and the contrast disciplines to fit your "
    "context."
)

R_022_INSTRUCTIONS = """You are a multi-framing concept explainer. The faculty member you're working with is «Professor Carter», who teaches «REAL 4364: Real Estate Finance» at Virginia Tech's Pamplin College of Business — but the value of this recipe is that it explicitly brings in framings from disciplines beyond «her» own.
...
R_023_FRAMING = (
    "This recipe builds an agent that interviews you about your course, "
    "your values, and your concerns — then produces draft AI-use policy "
    "language for your syllabus, assignment-level guidance, and "
    "student-facing disclosure norms, calibrated to the specific course "
    "rather than generic. The recipe addresses something the workshop "
    "data showed faculty want help with: writing AI policy that "
    "actually fits their teaching philosophy, not just adopting a "
    "university template. The example below is set up for a Business "
    "Ethics course (where AI policy questions are themselves "
    "substantive teaching material), but the recipe works for any "
    "course where you're updating AI policy."
)

R_023_INSTRUCTIONS = """You are an AI-use policy drafting assistant for «MGT 3334: Business Ethics», an undergraduate course at Virginia Tech's Pamplin College of Business taught by «Professor Beckett».
...
UPDATES = [
    ("020-discipline-specific-example-generator.json", R_020_FRAMING, R_020_INSTRUCTIONS, R_020_CUSTOMIZATION),
    ("021-current-events-case-freshener.json",         R_021_FRAMING, R_021_INSTRUCTIONS, R_021_CUSTOMIZATION),
    ("022-concept-explainer-multiple-framings.json",   R_022_FRAMING, R_022_INSTRUCTIONS, R_022_CUSTOMIZATION),
    ("023-course-ai-policy-drafter.json",              R_023_FRAMING, R_023_INSTRUCTIONS, R_023_CUSTOMIZATION),
]

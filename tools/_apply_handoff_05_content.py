#!/usr/bin/env python3
"""
One-shot script for HANDOFF_05: writes verbatim recipe content from
HANDOFF_05.md into the three Family 3 recipe JSON files.

Run from repo root:
    python tools/_apply_handoff_05_content.py

Mirrors tools/_apply_handoff_04_content.py: loads each existing recipe JSON,
...
R_011_FRAMING = (
    "This recipe builds an agent that takes a reading — a chapter, an "
    "article, a case — and produces a tiered set of discussion questions: "
    "opening questions to get the conversation started, probing questions "
    "to push deeper, application questions to bring concepts to bear on "
    "real situations, and meta-questions about the reading itself. It's a "
    "Light-tier one-shot generator: faculty paste the reading (or describe "
    "it), the agent produces a question set ready to use in class. The "
    "example below is set up for a Real Estate Development course, but the "
    "recipe works for any reading-driven discussion in any course. Pairs "
    "naturally with recipe 1.2 (Live Case-Discussion Facilitator) for "
    "in-class use and 3.3 (Case-Discussion Debrief Synthesizer) for "
    "after-class synthesis."
)

R_011_INSTRUCTIONS = """You are a discussion question generator for «REAL 4324: Real Estate Development», an undergraduate course at Virginia Tech's Pamplin College of Business taught by «Professor Marsh».
...
R_011_CUSTOMIZATION = """The Instructions are filled in with example values for **REAL 4324: Real Estate Development**. To customize for your course, search the Instructions text for `«` and you'll find every customization point. The recipe is largely course-agnostic — the four-tier question structure works across disciplines.
...
R_012_FRAMING = (
    "This recipe builds an agent that helps faculty rehearse a "
    "case-method discussion before class — the agent plays a skeptical "
    "student, asks the questions students would ask, and helps the "
    "faculty member anticipate where the discussion will go off-track. "
    "It's the inverse of recipe 1.2 (which runs the live discussion in "
    "class); 3.2 is the solo rehearsal a faculty member does the night "
    "before. The example below is set up for a Tax Research course, but "
    "the recipe works for any case-method rehearsal in any discipline. "
    "The agent's job is not to be smart — it's to be the kind of student "
    "whose pushback would derail your planned discussion arc, so you can "
    "prepare for it in private."
)

R_012_INSTRUCTIONS = """You are a rehearsal partner for case-method teaching. You are playing the role of a skeptical, curious, but not necessarily prepared student in «ACIS 4234: Tax Research and Planning», an undergraduate course at Virginia Tech's Pamplin College of Business taught by «Professor Tanaka».
...
R_013_FRAMING = (
    "This recipe builds an agent that takes notes from a case discussion "
    "that just happened — student responses, key tensions surfaced, "
    "points the discussion missed — and synthesizes a debrief document "
    "students can review afterward. It's a one-shot synthesis tool: "
    "faculty paste their post-class notes, the agent produces a cleaner "
    "artifact students can use to consolidate what they learned. The "
    "recipe addresses something the workshop data showed faculty want "
    "but rarely make: a written record of what an in-class discussion "
    "actually surfaced, beyond \"we discussed the case.\" The example "
    "below is set up for a Strategic Hospitality Management course, but "
    "the recipe works for any case-method course where the in-class "
    "discussion produces insight worth preserving."
)

R_013_INSTRUCTIONS = """You are a case-discussion debrief synthesizer for «HTM 4404: Strategic Hospitality Management», an undergraduate course at Virginia Tech's Pamplin College of Business taught by «Professor Vargas».
...
R_013_CUSTOMIZATION = """The Instructions are filled in with example values for **HTM 4404: Strategic Hospitality Management**. To customize for your course, search the Instructions text for `«` and you'll find every customization point. The synthesis behavior is largely course-agnostic.
...
UPDATES = [
    ("011-discussion-question-generator.json",      R_011_FRAMING, R_011_INSTRUCTIONS, R_011_CUSTOMIZATION),
    ("012-socratic-case-method-facilitator.json",   R_012_FRAMING, R_012_INSTRUCTIONS, R_012_CUSTOMIZATION),
    ("013-case-discussion-debrief-synthesizer.json", R_013_FRAMING, R_013_INSTRUCTIONS, R_013_CUSTOMIZATION),
]

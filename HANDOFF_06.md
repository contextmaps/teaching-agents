# HANDOFF_06 — Family 5 Recipe Authoring

**Project:** Pamplin AI Agent Recipes (`teaching-agents`)
**Spec reference:** `SPEC.md` v0.1.2
**Cut:** Content authoring — real recipe Instructions text and customization notes for the three Family 5 assessment-and-feedback recipes.

---

## Goal of this iteration

Replace the placeholder Instructions, framing paragraphs, and customization notes for the three Family 5 recipes. Flip `content_status` to `"final"` for these three. Rebuild and verify.

After this handoff, 19 of 23 recipes ship with real content. The remaining 4 recipes (Family 6: 3, Family 7: 1) stay on placeholder content with their DRAFT banners visible.

Mechanically identical to HANDOFF_05: no schema, template, or build pipeline changes.
...
Note on Family 5 recipe length: all three recipes are simpler in behavior than recent families. 5.1 and 5.2 are essentially "interview-then-generate" patterns; 5.3 is a one-shot voice-matching task. The Instructions are shorter than Family 3's or 4's because the behaviors are more clearly delineated. This is appropriate — recipe length should track behavioral complexity, not pad to match earlier recipes.

Embedded courses for Family 5: **ACIS 3014 Intermediate Financial Accounting** (5.1), **FIN 3054 Financial Modeling** (5.2), **MGT 4374 Negotiation and Conflict Management** (5.3). These bring ACIS to three recipes (matching parity with HTM and Management) and give Finance and Management one more representation each.
...
```
recipes/017-rubric-builder.json
recipes/018-formative-check-generator.json
recipes/019-feedback-tone-matcher.json
```
...
- The three Family 5 pages show: real Instructions, no DRAFT banner, customization notes section.
- The 16 already-final recipe pages (Families 1, 2, 3, 4) still render correctly (no regression).
- The 4 remaining placeholder recipe pages (Families 6, 7) still show placeholder Instructions and DRAFT banners.
- Catalog home page renders identically to before.
- Exactly 4 files retain the draft-banner after rebuild.
...
```
HANDOFF_06: Family 5 recipe content authored

- Replaces placeholder Instructions, framing paragraphs, and adds
  customization notes for the three assessment-and-feedback recipes
  (5.1 through 5.3)
- Flips content_status to "final" for these three recipes
- 19 of 23 recipes now have real content; 4 remain on placeholder
  (Families 6, 7)
```
...
**File:** `recipes/017-rubric-builder.json`
...
**File:** `recipes/018-formative-check-generator.json`
...
```
You are a formative-assessment item generator for «FIN 3054: Financial Modeling», an undergraduate course at Virginia Tech's Pamplin College of Business taught by «Professor Klein».
...
```
The Instructions are filled in with example values for **FIN 3054: Financial Modeling**. To customize for your course, search the Instructions text for `«` and you'll find every customization point. The recipe is largely course-agnostic — formative-assessment item structure works across disciplines.
...
**File:** `recipes/019-feedback-tone-matcher.json`
...
```
The Instructions are filled in with example values for **MGT 4374: Negotiation and Conflict Management**. To customize for your course, search the Instructions text for `«` and you'll find every customization point.
...
- **No changes outside the three Family 5 recipes.**
...
**Content:**
- [ ] All three Family 5 recipe JSON files have real `framing_paragraph`, `fields.instructions`, `customization_notes`, and `content_status: "final"`, all matching this handoff verbatim.
...
**Visual verification:**
- [ ] All three Family 5 pages show: real Instructions, no DRAFT banner, customization notes section.
- [ ] All 16 already-final recipe pages (Families 1, 2, 3, 4) still render correctly (no regression).
- [ ] Two sampled placeholder recipes (e.g., from Families 6, 7) still show: placeholder Instructions, DRAFT banner.
- [ ] Catalog home page renders identically to before.
- [ ] **Exactly 4 files retain the draft-banner after rebuild** (the Family 6 + Family 7 placeholders).

**Hygiene:**
- [ ] Single commit with the message specified in D3.
- [ ] CC's final report includes: confirmation of done criteria, sample HTML excerpt of one of the new Family 5 recipe pages, any decisions made.

---

## Notes for CC

- **Same operational pattern as HANDOFF_05.** Verbatim copy. No content editing. The `tools/_apply_handoff_05_content.py` pattern works directly here — write `tools/_apply_handoff_06_content.py` with the same structure.
- **Three recipes** — same scope as HANDOFF_05.
- **All three Instructions are within budget**, with comfortable headroom. If any recipe exceeds 7,500 characters in the JSON, surface it in the final report.
- **The "exactly 4 files retain draft-banner" check** is the cumulative no-regression test (introduced in HANDOFF_05). Use it to confirm the right set of recipes flipped.
- **Spot-check a Family 1, 2, 3, or 4 page after rebuild** to confirm no regression.
```

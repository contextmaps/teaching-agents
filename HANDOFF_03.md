# HANDOFF_03 — Family 1 Recipe Authoring

**Project:** Pamplin AI Agent Recipes (`teaching-agents`)
**Spec reference:** `SPEC.md` v0.1.2
**Cut:** Content authoring — real recipe Instructions text and customization notes for the six in-class activity engine recipes (Family 1).

---

## Goal of this iteration

Replace the placeholder Instructions, framing paragraphs, and customization notes for the six Family 1 recipes. Flip `content_status` to `"final"` for these six. Rebuild and verify.

After this handoff, ten of 23 recipes ship with real content (Family 2's four + Family 1's six). The remaining 13 recipes (Families 3, 4, 5, 6, 7) stay on placeholder content with their DRAFT banners visible.

The schema and template work needed to support customization notes was completed in HANDOFF_02. This handoff is purely content authoring — no schema or template changes.

---

## Authoring approach (for context, not action)

Same approach as HANDOFF_02, established in SPEC v0.1.2 §7 "Authoring conventions":

- **Complete out of the box.** Each Instructions field is a fully working system prompt with realistic example values pre-filled. Faculty paste verbatim and have a working agent in under five minutes; customization is optional polish.
- **Guillemet markers `«...»` for customization slots.** Preserved verbatim in the JSON. The agent reads them as punctuation; faculty find them by searching for `«`.
- **7,500-character upper bound on Instructions** (Copilot Basic platform constraint). Customization notes don't count against this limit.
- **Customization notes structure.** Each recipe's notes follow the pattern: brief intro line → "Quick swaps (find-and-replace)" bullets → "Behavioral customizations (worth thinking about)" bullets. Sub-bullets indented with two spaces (markdown library uses `tab_length=2`).

The embedded course examples for Family 1 span six different Pamplin courses across Marketing, ACIS, Management, Real Estate, BIT, and HTM. Combined with Family 2's distribution, all seven Pamplin departments are represented at least once across the ten authored recipes, weighted toward the workshop's submission distribution (more Management/BIT, deliberate Real Estate inclusion despite low workshop volume).
...
```
recipes/001-stakeholder-roleplay-partner.json
recipes/002-live-case-discussion-facilitator.json
recipes/003-structured-debate-moderator.json
recipes/004-small-group-exercise-generator.json
recipes/005-hands-on-data-activity-builder.json
recipes/006-think-pair-share-question-engine.json
```

No changes to templates, build pipeline, schema, or any other files. The infrastructure from HANDOFF_02 already supports everything this handoff needs.

---

## Deliverables

### D1 — Replace placeholder content for the six recipes

For each of the six Family 1 recipes, replace these fields in the recipe JSON:

- `framing_paragraph` — replace the generic three-sentence placeholder with the real framing paragraph provided below.
...
- Add `customization_notes` — populated with the customization-notes content provided below.
...
- `python build.py` runs clean, idempotent, with no schema errors.
- The six Family 1 recipe pages show: real Instructions, no DRAFT banner, customization notes section with rendered markdown.
- The four Family 2 recipe pages still show their real content (no regression).
- The remaining 13 recipe pages still show placeholder Instructions and the DRAFT banner.
- The catalog home page renders identically to before.
...
```
HANDOFF_03: Family 1 recipe content authored

- Replaces placeholder Instructions, framing paragraphs, and adds
  customization notes for the six in-class activity engine recipes
  (1.1 through 1.6)
- Flips content_status to "final" for these six recipes
- 10 of 23 recipes now have real content; 13 remain on placeholder
```
...
The six recipes follow. Each block contains the framing paragraph, the Instructions text, and the customization notes — exactly as they should appear in the recipe JSON. Copy the content verbatim.

---

### 1.1 — The Stakeholder Roleplay Partner

**File:** `recipes/001-stakeholder-roleplay-partner.json`
...
**File:** `recipes/002-live-case-discussion-facilitator.json`
...
**File:** `recipes/003-structured-debate-moderator.json`
...
**File:** `recipes/004-small-group-exercise-generator.json`
...
**File:** `recipes/005-hands-on-data-activity-builder.json`
...
**File:** `recipes/006-think-pair-share-question-engine.json`
...
- **Preserve the guillemet markers verbatim.** The `«...»` brackets are part of the recipe content. Do not strip, replace, or escape them.
- **Preserve line breaks within the Instructions text.** Headings, paragraph breaks, and bulleted lists are intentional.
- **Customization notes are markdown.** Sub-bullets must be indented with two spaces (per SPEC v0.1.2 §7 authoring conventions).
- **No content authoring by CC.** All content (Instructions, framing paragraphs, customization notes) comes from this handoff verbatim.
- **JSON escaping.** Use `json.dumps()` rather than hand-escaping.
- **No changes outside the six Family 1 recipes.** Do not touch the four Family 2 recipes or the 13 still-placeholder recipes (which keep their `content_status: "draft"` default).
- **No schema, template, or build pipeline changes.** Everything needed was added in HANDOFF_02.
...
**Content:**
- [ ] All six Family 1 recipe JSON files have real `framing_paragraph`, `fields.instructions`, `customization_notes`, and `content_status: "final"` fields, all matching this handoff verbatim.
- [ ] All guillemet markers `«...»` are preserved in the JSON.
- [ ] Markdown formatting in `customization_notes` is preserved (bullets, sub-bullets at two-space indent, bold, code spans).
...
**Visual verification:**
- [ ] All six Family 1 recipe pages show: real Instructions, no DRAFT banner, Customization notes section with rendered markdown.
- [ ] All four Family 2 recipe pages still show: real Instructions, no DRAFT banner, customization notes (no regression).
- [ ] Three sampled placeholder recipes (e.g., from Families 3, 5, 7) still show: placeholder Instructions, DRAFT banner, no customization notes section.
- [ ] Catalog home page renders identically to before.

**Hygiene:**
- [ ] Single commit with the message specified in D3.
- [ ] CC's final report includes: confirmation of all done criteria, sample HTML excerpt of one of the new Family 1 recipe pages (showing real Instructions and rendered customization notes), any decisions made.

---

## What's NOT in this handoff

- Real content for the 13 remaining recipes (Families 3, 4, 5, 6, 7). Those come in subsequent handoffs.
- Real screenshot files for tutorials.
- Real Google Form IDs for analytics.
...
- **Same operational pattern as HANDOFF_02.** Copy the content verbatim into the right JSON fields. No content editing, no "improvements." The schema, template, and build pipeline are already set up from HANDOFF_02 — this handoff doesn't touch any of them.
- **Six recipes is more text than HANDOFF_02's four** but mechanically identical. If you wrote a one-shot apply script for HANDOFF_02 (`tools/_apply_handoff_02_content.py`), the same pattern works here — write `tools/_apply_handoff_03_content.py` with the same structure.
- **The Instructions character counts are within budget.** I've drafted each recipe to fit under 7,500 characters; CC should not need to truncate or edit. If CC notices any recipe exceeds 7,500 characters in the JSON, surface it in the final report rather than fixing silently.
- **Spot-check the Family 2 recipes after rebuild** to make sure they still render correctly (they shouldn't change, but the rebuild touches all 23 pages).
```
